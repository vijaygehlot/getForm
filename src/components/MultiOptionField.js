...
function MultiOptionField({fieldModel, onSelected }){
    ...
    const addOption = opt => {
        ...
        if(fieldModel.type === "multi-option-multi-answer"){
        ...
        <div className="input">
            ...
            {fieldModel.options.map((option, index) => (
                <div className="input inline" key={index}>
                    <input type={fieldModel.type === "multi-option-multi-answer" ? "checkbox" : "radio"}  className="mr-1" name={fieldModel.title.replace(" ", "")} onChange={() => addOption(option)} />
                    <label>{option}</label>
                </div>
            ))}
        </div>
