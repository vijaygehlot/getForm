    ...
    return (
        <div>
            <div className="input">
                ...
                <input type="text" placeholder={`Eg. Select your ${inputType === "multi-option-multi-answer" ? "skills" : "gender"}`} onChange={e => setTitle(e.target.value)} />
            </div>
            { options.length > 0 && (
                <div className="mb-1">
                    ...
                    { options.map((opt, index) => (
                        <div className="input inline mb-0" key={index}>
                            <input type={inputType === "multi-option-single-answer" ? "radio" : "checkbox"} className="mr-1" name="inputs" />
                            <label>{opt}</label>
                        </div>
                    )) }
                </div>
